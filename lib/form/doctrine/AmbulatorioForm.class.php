 ...
 */
class AmbulatorioForm extends BaseAmbulatorioForm
{
  /**
   * @see CentroForm
   */
  public function configure()
  {
    ...
    $ids = (count($ids) > 0) ? $ids : array(0);
      $this->setWidget('estado_id', new sfWidgetFormDoctrineChoice(array(
                    ...
                    'onchange' => "cargarCombo('" . url_for('main/jsonMunicipios/') . "',this.value,'ambulatorio_municipio_id')"
                )));
        if ($this->isNew()) {
            $this->setWidget('municipio_id', new sfWidgetFormSelect(array('choices' => array('***** Seleccione *****')), array('onchange' => "cargarCombo('" . url_for('main/jsonParroquias/') . "',this.value,'ambulatorio_parroquia_id')")));
            ...
            $this->setWidget('municipio_id', new sfWidgetFormDoctrineChoice(array(
                        ...
                        'onchange' => "cargarCombo('" . url_for('main/jsonParroquias/') . "',this.value,'ambulatorio_parroquia_id')"
                    )));
