...
class sfDoctrinePagerLayout extends Doctrine_Pager_Layout {
    ...
    public function display($options = array(), $return = false) {
        $pager = $this->getPager();
        $str = '';
        $this->addMaskReplacement('page', image_tag("first.png", "alt=\"Primera Página\" title=\"Primera Página\" border=0"), true);
        $options['page_number'] = $pager->getFirstPage();
        $str .= $this->processPage($options);
        $this->addMaskReplacement('page', image_tag("previous.png", "alt=\"Página Anterior\" title=\"Página Anterior\" border=0"), true);
        $options['page_number'] = $pager->getPreviousPage();
        $str .= $this->processPage($options);
        $this->removeMaskReplacement('page');
        $str .= parent::display($options, true);
        $this->addMaskReplacement('page', image_tag("next.png", "alt=\"Página Siguiente\" title=\"Página Siguiente\" border=0"), true);
        $options['page_number'] = $pager->getNextPage();
        $str .= $this->processPage($options);
        $this->addMaskReplacement('page', image_tag("last.png", "alt=\"Última Página\" title=\"Última Página\" border=0"), true);
